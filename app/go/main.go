...
var sessionCache = isucache.NewMap[string, string]("session")
...
func getUserIDFromSession(c echo.Context) (string, int, error) {
	...
	jiaUserID, ok := sessionCache.Load(cookie.Value)
	if !ok {
...
var latestIsuConditionCache = isucache.NewAtomicMap[string, *IsuCondition]("latest_isu_condition")

func initLatestIsuConditionCache() error {
	var isuList []Isu
	...
	for _, isu := range isuList {
		...
		latestIsuConditionCache.Store(isu.JIAIsuUUID, &isuCondition)
	}
...
func getTrend(c echo.Context) error {
	res, err := trendCache.Get(c.Request().Context(), 0)
	if err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func isuConditionQueueWorker() {
	...
	LOOP:
		for {
			...
			cond, ok := latestIsuConditionCache.Load(req.JIAIsuUUID)
			if !ok || cond.Timestamp.Before(timestamp) {
